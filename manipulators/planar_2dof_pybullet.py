    ...
    def __init__(self, timestep, q0, qdot0, multimodel=False):
        ...
        for i in range(3):
            self.client.changeDynamics(
                0, i, lateralFriction=0.0, linearDamping=0.0, angularDamping=0.0
            )
        ...
        if self.multimodel:
            self.objects_params = [(0.1, 0.05), (0.01, 0.01), (1.0, 0.3)]
        self.i = 0
    ...
    def set_control(self, u):
        for i in range(2):
            self.client.setJointMotorControl2(
                0, i + 1, pybullet.TORQUE_CONTROL, **dict(force=u[i])
            )
